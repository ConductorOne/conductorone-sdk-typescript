...
<!-- UsageSnippet language="typescript" operationID="c1.api.task.v1.TaskSearchService.Search" method="post" path="/api/v1/search/tasks" -->
```typescript
...
import { taskSearchSearch } from "conductorone-sdk-typescript/funcs/taskSearchSearch.js";
...
async function run() {
  const res = await taskSearchSearch(conductoroneSDKTypescript);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("taskSearchSearch failed:", res.error);
  }
}
