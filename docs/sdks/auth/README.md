...
<!-- UsageSnippet language="typescript" operationID="c1.api.auth.v1.Auth.Introspect" method="get" path="/api/v1/auth/introspect" -->
```typescript
...
import { authIntrospect } from "conductorone-sdk-typescript/funcs/authIntrospect.js";
...
async function run() {
  const res = await authIntrospect(conductoroneSDKTypescript);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("authIntrospect failed:", res.error);
  }
}
