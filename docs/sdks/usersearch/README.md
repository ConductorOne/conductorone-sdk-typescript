...
<!-- UsageSnippet language="typescript" operationID="c1.api.user.v1.UserSearch.Search" method="post" path="/api/v1/search/users" -->
```typescript
...
import { userSearchSearch } from "conductorone-sdk-typescript/funcs/userSearchSearch.js";
...
async function run() {
  const res = await userSearchSearch(conductoroneSDKTypescript);
  if (res.ok) {
    ...
    console.log("userSearchSearch failed:", res.error);
  }
}
