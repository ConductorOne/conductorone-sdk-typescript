...
<!-- UsageSnippet language="typescript" operationID="c1.api.policy.v1.PolicySearch.Search" method="post" path="/api/v1/search/policies" -->
```typescript
...
async function run() {
  ...
  if (res.ok) {
    ...
    console.log("policySearchSearch failed:", res.error);
  }
}
