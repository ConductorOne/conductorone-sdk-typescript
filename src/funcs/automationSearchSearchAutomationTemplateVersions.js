 ...
 */
export function automationSearchSearchAutomationTemplateVersions(client, request, options) {
    return new APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = safeParse(request, (value) => shared.SearchAutomationTemplateVersionsRequest$outboundSchema.optional()
    ...
    const [result] = await M.match(M.json(200, operations
        .C1ApiAutomationsV1AutomationSearchServiceSearchAutomationTemplateVersionsResponse$inboundSchema, { key: "SearchAutomationTemplateVersionsResponse" }))(response, req, { extraFields: responseFields });
    if (!result.ok) {
