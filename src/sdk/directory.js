/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
import { directoryCreate } from "../funcs/directoryCreate.js";
import { directoryDelete } from "../funcs/directoryDelete.js";
import { directoryGet } from "../funcs/directoryGet.js";
import { directoryList } from "../funcs/directoryList.js";
import { ClientSDK } from "../lib/sdks.js";
import { unwrapAsync } from "./types/fp.js";
export class Directory extends ClientSDK {
    /**
     * List
     *
     * @remarks
     * List directories.
     */
    async list(request, options) {
        return unwrapAsync(directoryList(this, request, options));
    }
    /**
     * Create
     *
     * @remarks
     * Create a directory.
     */
    async create(request, options) {
        return unwrapAsync(directoryCreate(this, request, options));
    }
    /**
     * Delete
     *
     * @remarks
     * Delete a directory by app_id.
     */
    async delete(request, options) {
        return unwrapAsync(directoryDelete(this, request, options));
    }
    /**
     * Get
     *
     * @remarks
     * Get a directory by app_id.
     */
    async get(request, options) {
        return unwrapAsync(directoryGet(this, request, options));
    }
}
