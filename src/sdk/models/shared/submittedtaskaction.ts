 ...
 */
export type SubmittedTaskAction = {
  /**
   * The type of action that was performed.
   */
...
/** @internal */
export const SubmittedTaskAction$inboundSchema: z.ZodType<
  SubmittedTaskAction,
  z.ZodTypeDef,
  unknown
> = z.object({
...
export function submittedTaskActionFromJSON(
  jsonString: string,
): SafeParseResult<SubmittedTaskAction, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => SubmittedTaskAction$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'SubmittedTaskAction' from JSON`,
  );
}
